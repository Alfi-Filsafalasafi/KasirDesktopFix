Imports System.Data.SqlClient
Imports Microsoft.Office.Interop
Public Class Export
    Dim da As SqlDataAdapter
    Dim ds As DataSet
    Dim cmd As SqlCommand

    Private Sub getBarang()
        Call konek_db()
        Dim sql As String

        sql = "SELECT kd_barang,barcode,nama_barang,kategori,satuan,nama_supplier,stok,harga_beli,harga_jual,expired FROM t_barang inner join t_satuan on t_barang.id_satuan = t_satuan.id_satuan inner join t_kategori on t_barang.id_kategori = t_kategori.id_kategori inner join t_supplier on t_barang.id_supplier = t_supplier.id_supplier where kd_barang > 4005 and kd_barang <= 6047" '6047
        da = New SqlDataAdapter(sql, koneksi)
        ds = New DataSet
        da.Fill(ds, 0)
        dgv.DataSource = ds.Tables(0)
        dgv.Columns(0).Width = "80"
        dgv.Columns(0).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        dgv.Columns(6).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        dgv.Columns(7).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        dgv.Columns(9).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
        dgv.ReadOnly = True
    End Sub

    Private Sub getKategori()
        Call konek_db()
        Dim sql As String

        sql = "SELECT * FROM t_kategori"
        da = New SqlDataAdapter(sql, koneksi)
        ds = New DataSet
        da.Fill(ds, 0)
        dgv.DataSource = ds.Tables(0)
        dgv.Columns(0).Width = "80"
        dgv.Columns(0).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        dgv.ReadOnly = True
    End Sub

    Private Sub getSupplier()
        Call konek_db()
        Dim sql As String

        sql = "SELECT * FROM t_supplier"
        da = New SqlDataAdapter(sql, koneksi)
        ds = New DataSet
        da.Fill(ds, 0)
        dgv.DataSource = ds.Tables(0)
        dgv.Columns(0).Width = "80"
        dgv.Columns(0).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        dgv.ReadOnly = True
    End Sub

    Private Sub getSatuan()
        Call konek_db()
        Dim sql As String

        sql = "SELECT * FROM t_satuan"
        da = New SqlDataAdapter(sql, koneksi)
        ds = New DataSet
        da.Fill(ds, 0)
        dgv.DataSource = ds.Tables(0)
        dgv.Columns(0).Width = "80"
        dgv.Columns(0).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
        dgv.ReadOnly = True
    End Sub

    Private Sub getData()
        getBarang()
        'getKategori()
        'getSupplier()
        'getSatuan()
    End Sub
    Private Sub Export_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call getData()
    End Sub

    Private Sub BarangExcel()
        Dim xa As New Excel.Application
        Dim ws As Excel.Worksheet
        Dim wb As Excel.Workbook

        wb = xa.Workbooks.Add()
        ws = xa.Worksheets("Sheet1")

        ws.Cells(1, 1) = dgv.Columns(0).HeaderText
        ws.Cells(1, 2) = dgv.Columns(1).HeaderText
        ws.Cells(1, 3) = dgv.Columns(2).HeaderText
        ws.Cells(1, 4) = dgv.Columns(3).HeaderText
        ws.Cells(1, 5) = dgv.Columns(4).HeaderText
        ws.Cells(1, 6) = dgv.Columns(5).HeaderText
        ws.Cells(1, 7) = dgv.Columns(6).HeaderText
        ws.Cells(1, 8) = dgv.Columns(7).HeaderText
        ws.Cells(1, 9) = dgv.Columns(8).HeaderText
        ws.Cells(1, 10) = dgv.Columns(9).HeaderText

        For i As Integer = 0 To dgv.RowCount - 1
            Dim dgvExcel As DataGridViewRow = dgv.Rows(i)

            ws.Cells(2 + i, 1) = dgvExcel.Cells(0).Value
            ws.Cells(2 + i, 2) = dgvExcel.Cells(1).Value
            ws.Cells(2 + i, 3) = dgvExcel.Cells(2).Value
            ws.Cells(2 + i, 4) = dgvExcel.Cells(3).Value
            ws.Cells(2 + i, 5) = dgvExcel.Cells(4).Value
            ws.Cells(2 + i, 6) = dgvExcel.Cells(5).Value
            ws.Cells(2 + i, 7) = dgvExcel.Cells(6).Value
            ws.Cells(2 + i, 8) = dgvExcel.Cells(7).Value
            ws.Cells(2 + i, 9) = dgvExcel.Cells(8).Value
            ws.Cells(2 + i, 10) = dgvExcel.Cells(9).Value

        Next
        xa.Visible = True
    End Sub

    Private Sub SatuanExcel()
        Dim xa As New Excel.Application
        Dim ws As Excel.Worksheet
        Dim wb As Excel.Workbook

        wb = xa.Workbooks.Add()
        ws = xa.Worksheets("Sheet1")

        ws.Cells(1, 1) = dgv.Columns(0).HeaderText
        ws.Cells(1, 2) = dgv.Columns(1).HeaderText

        For i As Integer = 0 To dgv.RowCount - 1
            Dim dgvExcel As DataGridViewRow = dgv.Rows(i)

            ws.Cells(2 + i, 1) = dgvExcel.Cells(0).Value
            ws.Cells(2 + i, 2) = dgvExcel.Cells(1).Value

        Next
        xa.Visible = True
    End Sub

    Private Sub KategoriExcel()
        Dim xa As New Excel.Application
        Dim ws As Excel.Worksheet
        Dim wb As Excel.Workbook

        wb = xa.Workbooks.Add()
        ws = xa.Worksheets("Sheet1")

        ws.Cells(1, 1) = dgv.Columns(0).HeaderText
        ws.Cells(1, 2) = dgv.Columns(1).HeaderText

        For i As Integer = 0 To dgv.RowCount - 1
            Dim dgvExcel As DataGridViewRow = dgv.Rows(i)

            ws.Cells(2 + i, 1) = dgvExcel.Cells(0).Value
            ws.Cells(2 + i, 2) = dgvExcel.Cells(1).Value

        Next
        xa.Visible = True
    End Sub

    Private Sub SupplierExcel()
        Dim xa As New Excel.Application
        Dim ws As Excel.Worksheet
        Dim wb As Excel.Workbook

        wb = xa.Workbooks.Add()
        ws = xa.Worksheets("Sheet1")

        ws.Cells(1, 1) = dgv.Columns(0).HeaderText
        ws.Cells(1, 2) = dgv.Columns(1).HeaderText
        ws.Cells(1, 3) = dgv.Columns(2).HeaderText
        ws.Cells(1, 4) = dgv.Columns(3).HeaderText
        ws.Cells(1, 5) = dgv.Columns(4).HeaderText

        For i As Integer = 0 To dgv.RowCount - 1
            Dim dgvExcel As DataGridViewRow = dgv.Rows(i)

            ws.Cells(2 + i, 1) = dgvExcel.Cells(0).Value
            ws.Cells(2 + i, 2) = dgvExcel.Cells(1).Value
            ws.Cells(2 + i, 3) = dgvExcel.Cells(2).Value
            ws.Cells(2 + i, 4) = dgvExcel.Cells(3).Value
            ws.Cells(2 + i, 5) = dgvExcel.Cells(4).Value

        Next
        xa.Visible = True
    End Sub

    Private Sub btn_export_Click(sender As Object, e As EventArgs) Handles btn_export.Click
        'SupplierExcel()
        'KategoriExcel()
        'SatuanExcel()
        BarangExcel()
    End Sub
End Class